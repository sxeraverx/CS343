...
bool MethodMoveTransform::TraverseDecl(Decl *D)
{
  if (NamespaceDecl* N = dyn_cast<NamespaceDecl>(D)) {
    ...
    llvm::outs() << ", body? " << M->hasBody() 
      << ", inlined? " << M->isInlined()
      << ", isInlineSpecified? " << M->isInlineSpecified()
      << ", from: ";
    ...
    // TODO: Handle the case where a method has an inline body outside of the declaration
    ...
    if (M->hasBody()) {
      ...
      if (fn.rfind(".h") != std::string::npos && fn.find("/usr") == std::string::npos) {
        ...
        // grab everything after the declared name
        DeclarationNameInfo DNI = M->getNameInfo();
        SourceLocation NEL = sema->getPreprocessor().getLocForEndOfToken(DNI.getLoc());      
        FunctionDecl::param_iterator PB = M->param_begin();
        FunctionDecl::param_iterator PE = M->param_end();
        FunctionDecl::param_iterator LPE = PB;
        
        SourceLocation DB = NEL;
      
      
        PreprocessorLexer *L = sema->getPreprocessor().getCurrentLexer();
        llvm::outs() << "current lexer: " << (void *)L;
        L = sema->getPreprocessor().getCurrentFileLexer();
        llvm::outs() << ", current file lexer: " << (void *)L << "\n";
      
        // handle the parameter list
        // TODO: filter out the default value expressions
        sst << "(";
        ...
        // get the last token location of the type
        TypeSourceInfo *TSI = M->getTypeSourceInfo();
        TypeLoc TL = TSI->getTypeLoc();
        SourceLocation TLE = sema->getPreprocessor().getLocForEndOfToken(TL.getEndLoc());
        llvm::outs() << ", last type location: ";
        TLE.print(llvm::outs(), sema->getSourceManager());
        llvm::outs() << "\n";

        if (CXXConstructorDecl* C = dyn_cast<CXXConstructorDecl>(D)) {
          auto IB = C->init_begin();
          auto IE = C->init_end();

          if (IB != IE) {
            // find the : token
            SourceLocation CL = TLE.getLocWithOffset(1);
            
            /* Lexer::findLocationAfterToken(TL.getEndLoc(), clang::tok::colon, sema->getSourceManager(), sema->getLangOpts(), true);

            llvm::outs() << "Initializer colon location: ";
            CL.print(llvm::outs(), sema->getSourceManager());
            llvm::outs() << "\n";

            CL = CL.getLocWithOffset(-1); */
            llvm::outs() << "Initializer colon location after offset: ";
            CL.print(llvm::outs(), sema->getSourceManager());
            llvm::outs() << "\n";
            ...
            const char *IDB = sema->getSourceManager().getCharacterData(CL);
            const char *IDE = sema->getSourceManager().getCharacterData((*LI)->getSourceRange().getEnd());
            std::string IT(IDB, IDE - IDB + 1);
            sst << IT;
            rewriter.RemoveText(SourceRange(CL, (*LI)->getSourceRange().getEnd()));
          }
        }
        ...
        if (LPE != PE) {
          NEL = (*LPE)->getSourceRange().getEnd();
        }
        
        // get the location of the last )
        SourceLocation LRPL = Lexer::findLocationAfterToken(NEL, clang::tok::r_paren, sema->getSourceManager(), sema->getLangOpts(), true);
        llvm::outs() << "Last r_paren location: ";
        LRPL.print(llvm::outs(), sema->getSourceManager());
        llvm::outs() << "\n";
        ...
        if (M->isInlineSpecified()) {
          // remove the whole inline method
          B = R.getBegin();
          rewriter.RemoveText(SourceRange(B, RBL));
        }
        else {
          // get the text of the last type token
          const char *TDB = sema->getSourceManager().getCharacterData(TL.getEndLoc());
          const char *TDE = sema->getSourceManager().getCharacterData(TLE);
          std::string LTT(TDB, TDE - TDB + 1);
          LTT += ";";
          
          llvm::outs() << "LTT = " << LTT << "\n";
          
          rewriter.ReplaceText(SourceRange(TL.getEndLoc(), TLE), LTT);
          rewriter.RemoveText(SourceRange(LBL, RBL));
        }
